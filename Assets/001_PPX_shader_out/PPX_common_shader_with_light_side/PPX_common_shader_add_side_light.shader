...
//技术谈论Q群:755239075
//最后,玩的开心!!!
Shader "Effect/PPX_FX/PPX_common_shader_add_side_light"
{
	Properties
	{
		...
		_alpha_power("alpha_power", Range( 0 , 11)) = 1
		_Fade_distance("Fade_distance", Range( 0 , 1111)) = 1
		...
		_Emissive_power("Emissive_power", Range( 0 , 3)) = 1.449342
		...
		_edge_hardness("edge_hardness", Range( 0 , 55)) = 1
		_dissolve("dissolve", Range( 0 , 1)) = 0.5023768
		...
		_Dissolve_side("Dissolve_side", Range( 0 , 0.5)) = 0.05378347
		[HDR]_Side_color("Side_color", Color) = (1,0,0,0)
		_Side_color_power("Side_color_power", Range( 0 , 3)) = 3
	...
	LOD 100
		...
		Pass
		{
			...
			uniform float _Side_color_power;
			uniform float _Dissolve_side;
			...
			v2f vert ( appdata v )
			{
				...
				float4 break11_g116 = _displace_speed;
				float mulTime5_g116 = _Time.y * break11_g116.z;
				float2 appendResult6_g116 = (float2(break11_g116.x , break11_g116.y));
				float4 break10_g116 = _displace_uv;
				float2 appendResult2_g116 = (float2(break10_g116.x , break10_g116.y));
				float2 appendResult3_g116 = (float2(break10_g116.z , break10_g116.w));
				float2 texCoord4_g116 = v.ase_texcoord * appendResult2_g116 + appendResult3_g116;
				float2 panner1_g116 = ( mulTime5_g116 * appendResult6_g116 + texCoord4_g116);
				float4 break11_g115 = _displace_mask_speed;
				float mulTime5_g115 = _Time.y * break11_g115.z;
				float2 appendResult6_g115 = (float2(break11_g115.x , break11_g115.y));
				float4 break10_g115 = _displace_mask_uv;
				float2 appendResult2_g115 = (float2(break10_g115.x , break10_g115.y));
				float2 appendResult3_g115 = (float2(break10_g115.z , break10_g115.w));
				float2 texCoord4_g115 = v.ase_texcoord.xy * appendResult2_g115 + appendResult3_g115;
				float2 panner1_g115 = ( mulTime5_g115 * appendResult6_g115 + texCoord4_g115);
				...
				#endif
				float3 displace516 = ( ( v.ase_normal * (( tex2Dlod( _displace_tex, float4( panner1_g116, 0, 0.0) ) * tex2Dlod( _displace_mask_tex, float4( panner1_g115, 0, 0.0) ) )).rgb ) * staticSwitch515 );
			...
			fixed4 frag (v2f i , half ase_vface : VFACE) : SV_Target
			{
				...
				float2 temp_output_523_0 = panner1_g96;
				...
				float2 staticSwitch535 = ( temp_output_523_0 + appendResult536 );
				#else
				float2 staticSwitch535 = temp_output_523_0;
				#endif
				...
				float4 tex2DNode419 = tex2D( _Distort_mask, uv_Distort_mask );
				float2 appendResult555 = (float2(tex2DNode419.r , tex2DNode419.g));
				#ifdef _USE_DISTORT_ON
				float2 staticSwitch543 = ( appendResult417 * staticSwitch445 * appendResult555 );
				#else
				...
				float3 hsvTorgb2_g111 = RGBToHSV( (tex2DNode71).xyz );
				float3 appendResult5_g111 = (float3(_HUE , _Saturation , _Value));
				float3 break6_g111 = appendResult5_g111;
				float3 hsvTorgb1_g111 = HSVToRGB( float3(( hsvTorgb2_g111.x + break6_g111.x ),( hsvTorgb2_g111.y + break6_g111.y ),( hsvTorgb2_g111.z + break6_g111.z )) );
				#ifdef _USE_CHANGE_COLOR_ON
				float4 staticSwitch520 = float4( hsvTorgb1_g111 , 0.0 );
				#else
				...
				float4 break11_g109 = _dissolve_speed;
				float mulTime5_g109 = _Time.y * break11_g109.z;
				float2 appendResult6_g109 = (float2(break11_g109.x , break11_g109.y));
				float4 break10_g109 = _dissolve_uv;
				float2 appendResult2_g109 = (float2(break10_g109.x , break10_g109.y));
				float2 appendResult3_g109 = (float2(break10_g109.z , break10_g109.w));
				float2 texCoord4_g109 = i.ase_texcoord1.xy * appendResult2_g109 + appendResult3_g109;
				float2 panner1_g109 = ( mulTime5_g109 * appendResult6_g109 + texCoord4_g109);
				float4 tex2DNode138 = tex2D( _dissolve_texture, panner1_g109 );
				float mul449 = tex2DNode138.r;
				float temp_output_7_0_g110 = _edge_hardness;
				float4 texCoord320 = i.ase_texcoord1;
				texCoord320.xy = i.ase_texcoord1.xy * float2( 1,1 ) + float2( 0,0 );
				float temp_output_574_0 = ( 1.0 - texCoord320.z );
				#ifdef _USE_CUSTOM1_X_DISSOLVE_ON
				float staticSwitch212 = temp_output_574_0;
				#else
				float staticSwitch212 = _dissolve;
				#endif
				float lerpResult2_g110 = lerp( temp_output_7_0_g110 , -1.0 , staticSwitch212);
				float temp_output_185_0 = saturate( ( ( tex2DNode138.r * temp_output_7_0_g110 ) - lerpResult2_g110 ) );
				float dissolve448 = saturate( temp_output_185_0 );
				...
				float4 break11_g117 = _color_speed;
				float mulTime5_g117 = _Time.y * break11_g117.z;
				float2 appendResult6_g117 = (float2(break11_g117.x , break11_g117.y));
				float4 break10_g117 = _color_uv;
				float2 appendResult2_g117 = (float2(break10_g117.x , break10_g117.y));
				float2 appendResult3_g117 = (float2(break10_g117.z , break10_g117.w));
				float2 texCoord4_g117 = i.ase_texcoord1.xy * appendResult2_g117 + appendResult3_g117;
				float2 panner1_g117 = ( mulTime5_g117 * appendResult6_g117 + texCoord4_g117);
				float3 desaturateInitialColor547 = tex2D( _color_Tex, panner1_g117 ).rgb;
				...
				#endif
				float temp_output_7_0_g118 = _edge_hardness;
				#ifdef _USE_CUSTOM1_X_DISSOLVE_ON
				float staticSwitch562 = ( temp_output_574_0 - _Dissolve_side );
				#else
				float staticSwitch562 = ( _dissolve - _Dissolve_side );
				#endif
				float lerpResult2_g118 = lerp( temp_output_7_0_g118 , -1.0 , staticSwitch562);
				float side_mask567 = ( temp_output_185_0 - saturate( ( ( tex2DNode138.r * temp_output_7_0_g118 ) - lerpResult2_g118 ) ) );
				float4 lerpResult569 = lerp( float4( staticSwitch497 , 0.0 ) , ( _Side_color * _Side_color_power ) , side_mask567);
				float4 screenPos500 = i.ase_texcoord3;
				...
				float temp_output_9_0_g119 = saturate( ( (staticSwitch460).w * distanceDepth500 ) );
				float4 appendResult2_g119 = (float4(( lerpResult569.rgb * temp_output_9_0_g119 ) , temp_output_9_0_g119));
				float4 appendResult6_g119 = (float4(appendResult2_g119.xyz , ( temp_output_9_0_g119 * _add_or_blend )));
				
				
				finalColor = appendResult6_g119;
				return finalColor;
			}
			ENDCG
		}
	}
	CustomEditor "PPX_common_light_side_shader_GUI2"
