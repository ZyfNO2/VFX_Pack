...
public class PPX_common_light_side_add_wpo_pos_shader_GUI : ShaderGUI
{
    ...
    MaterialProperty _dissolve_speed;
        ...
        MaterialProperty _mul_dissolve_texture;

    MaterialProperty _mul_dissolve_uv;

    MaterialProperty _mul_dissolve_speed;
    ...
    MaterialProperty _Dissolve_side;
MaterialProperty _Side_color;
MaterialProperty _Side_color_power;
    ...
    MaterialProperty _use_custom1_w_displace;
        ...
        MaterialProperty _Base_displace_tex;

    MaterialProperty _Base_displace_uv;

    MaterialProperty _Base_displace_speed;

    MaterialProperty _Base_direction;

    MaterialProperty _Base_displace_mask_tex;

    MaterialProperty _Base_displace_mask_uv;

    MaterialProperty _Base_displace_mask_speed;
...
.LabelField("流速:xy(-1,1)控制方向,z控制速度,w无作用", EditorStyles.miniButton);
        ...
        EditorGUILayout
.LabelField("默认和粒子系统中的custom1.y关联", EditorStyles.miniButton);
        ...
        if (_use_second_tex.floatValue == 1)
        {
                ...
                _mul_dissolve_texture = FindProperty("_mul_dissolve_texture", properties);
            materialEditor
                .TexturePropertySingleLine(new GUIContent("遮罩贴图"),
                _mul_dissolve_texture);
            _mul_dissolve_uv = FindProperty("_mul_dissolve_uv", properties);
            materialEditor.ShaderProperty(_mul_dissolve_uv, "遮罩uv");
            _mul_dissolve_speed = FindProperty("_mul_dissolve_speed", properties);
            materialEditor.ShaderProperty(_mul_dissolve_speed, "遮罩流速");
            ...
            // {
                ...
                materialEditor
                    ...
                    "使用custom1_x_溶解");
 _Dissolve_side = FindProperty("_Dissolve_side", properties);
                materialEditor.ShaderProperty(_Dissolve_side, "溶解边缘宽度");

                 _Side_color = FindProperty("_Side_color", properties);
                materialEditor.ShaderProperty(_Side_color, "溶解边缘颜色");

                 _Side_color_power = FindProperty("_Side_color_power", properties);
                materialEditor.ShaderProperty(_Side_color_power, "溶解边缘颜色强度");
        ...
        EditorGUILayout
            .LabelField("方向偏移", EditorStyles.miniButton);
_Base_displace_tex = FindProperty("_Base_displace_tex", properties);
            materialEditor
                .TexturePropertySingleLine(new GUIContent("位移贴图"),
                _Base_displace_tex);
            _Base_displace_uv = FindProperty("_Base_displace_uv", properties);
            materialEditor.ShaderProperty(_Base_displace_uv, "位移uv");
            _Base_displace_speed = FindProperty("_Base_displace_speed", properties);
            materialEditor.ShaderProperty(_Base_displace_speed, "位移流速");

            _Base_displace_mask_tex = FindProperty("_Base_displace_mask_tex", properties);
            materialEditor
                .TexturePropertySingleLine(new GUIContent("位移遮罩"),
                _Base_displace_mask_tex);
            _Base_displace_mask_uv = FindProperty("_Base_displace_mask_uv", properties);
            materialEditor.ShaderProperty(_Base_displace_mask_uv, "位移遮罩uv");
            _Base_displace_mask_speed =
                FindProperty("_Base_displace_mask_speed", properties);
            materialEditor.ShaderProperty(_Base_displace_mask_speed, "位移遮罩流速");
            _Base_direction = FindProperty("_Base_direction", properties);
            materialEditor.ShaderProperty(_Base_direction, "位移强度");
